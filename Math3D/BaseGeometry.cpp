...
Rect::Rect(const Vector2& pos, const Size& dimension)
{
...
bool Rect::intersectsCircle(const Vector2& center, float radius) const
{
    Vector2 rectangleCenter((origin.x + size.width / 2),
                         (origin.y + size.height / 2));
    
    float w = size.width / 2;
    float h = size.height / 2;
    
    float dx = std::fabs(center.x - rectangleCenter.x);
    float dy = std::fabs(center.y - rectangleCenter.y);
    ...
    Vector2 circleDistance(std::fabs(center.x - origin.x - w),
                        std::fabs(center.y - origin.y - h));
