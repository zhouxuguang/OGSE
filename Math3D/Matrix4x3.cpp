...
void	Matrix4x3::fromQuaternion(const Quaternion &q) 
{
	...
	float	ww = 2.0f * q.m_dfW;
	float	xx = 2.0f * q.m_dfX;
	float	yy = 2.0f * q.m_dfY;
	float	zz = 2.0f * q.m_dfZ;

	// Set the matrix elements.  There is still a little more
	// opportunity for optimization due to the many common
	// subexpressions.  We'll let the compiler handle that...

	m11 = 1.0f - yy*q.m_dfY - zz*q.m_dfZ;
	m12 = xx*q.m_dfY + ww*q.m_dfZ;
	m13 = xx*q.m_dfZ - ww*q.m_dfX;

	m21 = xx*q.m_dfY - ww*q.m_dfZ;
	m22 = 1.0f - xx*q.m_dfX - zz*q.m_dfZ;
	m23 = yy*q.m_dfZ + ww*q.m_dfX;

	m31 = xx*q.m_dfZ + ww*q.m_dfY;
	m32 = yy*q.m_dfZ - ww*q.m_dfX;
	m33 = 1.0f - xx*q.m_dfX - yy*q.m_dfY;
